...
def getleechinfo(from_user):
    ...
    if prefix != "Not Exists":
        buttons.sbutton("✔️Delete Prename", f"leechset {user_id} prename")
    if suffix != "Not Exists":
        buttons.sbutton("✔️Delete Suffix", f"leechset {user_id} suffix")
    if caption != "Not Exists": 
        buttons.sbutton("✔️Delete Caption", f"leechset {user_id} cap")
    if dumpid != "Not Exists":
        buttons.sbutton("✔️Delete DumpID", f"leechset {user_id} dump")
    if remname != "Not Exists": 
        buttons.sbutton("✔️Delete Remname", f"leechset {user_id} rem")
    if cfont != "Not Exists": 
        buttons.sbutton("✔️Delete CapFont", f"leechset {user_id} cfont")
...
def setLeechType(update, context):
    ...
    if user_id != int(data[1]):
        ...
        query.answer(text="✔️Your File Will Deliver As Document!", show_alert=True)
        ...
        query.answer(text="✔️Your File Will Deliver As Media!", show_alert=True)
        editLeechType(message, query)
        ...
        if ospath.lexists(path):
            ...
            query.answer(text="✔️Thumbnail Removed!", show_alert=True)
            editLeechType(message, query)
        else:
            query.answer(text="✔️Old Settings", show_alert=True)
        ...
        else: query.answer(text="✔️Send new settings command.")
    elif data[2] == "prename":
        PRE_DICT.pop(user_id)
        if DB_URI: 
            DbManger().user_pre(user_id, '')
        query.answer(text="✔️Your Prename is Successfully Deleted!", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "suffix":
        SUF_DICT.pop(user_id)
        if DB_URI: 
            DbManger().user_suf(user_id, '')
        query.answer(text="✔️Your Suffix is Successfully Deleted!", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "cap":
        CAP_DICT.pop(user_id)
        if DB_URI:
            DbManger().user_cap(user_id, None)
        query.answer(text="✔️Your Caption is Successfully Deleted!", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "rem":
        REM_DICT.pop(user_id)
        if DB_URI:
            DbManger().user_rem(user_id, None)
        query.answer(text="✔️Your Remname is Successfully Deleted!", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "dump":
        LEECH_DICT.pop(user_id)
        if DB_URI:
            DbManger().user_dump(user_id, None)
        query.answer(text="✔️Your Dump ID is Successfully Deleted!", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "cfont":
        CFONT_DICT.pop(user_id)
        if DB_URI:
            DbManger().user_cfont(user_id, None)
        query.answer(text="✔️Your CapFont is Successfully Deleted!", show_alert=True)
        editLeechType(message, query)
    else:
        query.answer()
        try:
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        sendMessage("✔️Hey Reply to a photo to save custom thumbnail.", context.bot, update.message)
