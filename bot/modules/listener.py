...
class MirrorLeechListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name):
        ...
        if BOT_PM and self.message.chat.type != 'private':
            if EMOJI_THEME is True:
                pmwarn = f"<b>⇛ Hey sent files in PM.</b>\n"
            else:
        ...
        if MIRROR_LOGS and self.message.chat.type != 'private':
            if EMOJI_THEME is True:
                logwarn = f"<b>⇛ Hey sent files in Mirror Log Channel. Join <a href=\"{MIRROR_LOG_URL}\">Mirror Log channel</a> </b>\n"
            else:
        ...
        if LEECH_LOG and self.message.chat.type != 'private':
            if EMOJI_THEME is True:
                logleechwarn = f"<b>⇛ Hey sent files in Leech Log Channel. Join <a href=\"{LEECH_LOG_URL}\">Leech Log channel</a> </b>\n"
            else:
    ...
    def onDownloadError(self, error):
        ...
        with download_dict_lock:
            try:
                ...
                LOGGER.error(str(e))
            count = len(download_dict)
        msg = f"{self.tag} ⇛ Hey your download has been stopped due to: {error}"
