...
def rss_list(update, context):
    if len(rss_dict) > 0:
        ...
        for title, url in list(rss_dict.items()):
            list_feed += f"<b>टाइटिल ⇛</b> <code>{title}</code>\n<b>फीड यूआरएल ⇛ </b><code>{url[0]}</code>\n\n"
...
def rss_sub(update, context):
    try:
        ...
        try:
            ...
            sub_msg += f"\n\n<b>❄️ टाइटल ⇛ </b><code>{title}</code>\n<b>Feed Url: </b>{feed_link}"
            sub_msg += f"\n\n<b>लेटेस्ट रिकॉर्ड फॉर </b>{rss_d.feed.title}:"
            sub_msg += f"\n\n<b>❄️ नाम ⇛ </b><code>{rss_d.entries[0]['title'].replace('>', '').replace('<', '')}</code>"
            try:
                link = rss_d.entries[0]['links'][1]['href']
            except IndexError:
                link = rss_d.entries[0]['link']
            sub_msg += f"\n\n<b>❄️ लिंक ⇛ </b><code>{link}</code>"
            sub_msg += f"\n\n<b>❄️ फिल्टर्स ⇛ </b><code>{filters}</code>"
            ...
            LOGGER.info(f"Rss फीड एडेड ⇛ {title} - {feed_link} - {filters}")
        ...
        try:
            ...
            while True:
                ...
                if RSS_COMMAND is not None:
                    ...
                    feed_msg += f"<b>❄️ नाम ⇛ </b>{rss_d.entries[feed_count]['title'].replace('>', '').replace('<', '')}"
                    #feed_msg += f"\n\n<b>❄️ नाम ⇛ </b><code>{rss_d.entries[feed_count]['title'].replace('>', '').replace('<', '')}</code>"
                else:
            ...
            LOGGER.info(f"फीड नाम ⇛ {name}")
            LOGGER.info(f"लॉस्ट आईटम ⇛ {last_link}")
        except Exception as e:
            LOGGER.error(f"{e} फीड नाम ⇛ {name} - फीड लिंक ⇛ {data[0]}")
            continue
